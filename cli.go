...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Dreamcord")
	var updateFlag = flag.Bool("repair", false, "Repair Dreamcord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Dreamcord")
	...
	if *versionFlag {
		fmt.Println("Dreamlotl Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Vendicated, thororen1234, Vencord, and Dreamcord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Dreamlotl' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Dreamcord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Dreamcord will likely not work.")
}
